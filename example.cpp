...
int main() {
    ...
    // multibuffer allocator is ok
    try {
        std::vector<std::vector<std::byte>> buffers(2, std::vector<std::byte>(k_elements_count * 128));
        cmba::cmb_resource resource1(buffers[0].data(), buffers[0].size());
        cmba::cmb_resource resource2(buffers[1].data(), buffers[1].size());
        std::vector<cmba::cmb_resource*> resources = {&resource1, &resource2};

        cmba::cmb_multiresource<std::allocator<cmba::cmb_resource*>> multiresource(resources);
        cmba::cmb_multiallocator<int, std::allocator<cmba::cmb_resource*>> allocator_int(&multiresource);
